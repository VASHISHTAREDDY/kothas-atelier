...
;(function($){
  $(document).on("click", "button.loadpage, .caption button", function(e){
    ...
    if (!link || link === "#" || link === "/" || tarP === curP || link === window.location.href) {
      ...
      var d = $(window).width() < 767 ? (window.headHeight - 57) : window.headHeight;
      if (typeof window.headHeight === "number" && window.headHeight > 0) {
        d = $(window).width() < 767 ? (window.headHeight - 57) : window.headHeight;
      }
      if (typeof TweenMax !== "undefined") {
        TweenMax.to(window, 2, { scrollTo: { y: d, autoKill: false, rotation: 0.01 }, ease: Cubic.easeInOut, onComplete: function() {
          if ($.pnp && $.pnp.enableScroll) $.pnp.enableScroll();
        }});
      } else {
        $("html, body").stop().animate({ scrollTop: d }, 1500);
      }
    }
  });
})(jQuery);
